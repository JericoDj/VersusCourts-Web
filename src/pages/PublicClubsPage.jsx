...
export default function PublicClubsPage() {
  ...
  return (
    ...
    >
      {results.length ? <div className="cards-grid cards-grid--clubs">{results.map((club) => <ClubCard club={club} key={club.id} />)}</div> : <div className="empty-state" style={{ '--empty-color': accent }}><span><ShieldCheck size={30} /></span><h3>No clubs match those filters</h3><p>Try another sport or search area.</p><button type="button" onClick={clear}>Clear filters</button></div>}
    </DirectoryLayout>
  )
}
